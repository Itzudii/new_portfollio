...
export class Book {
    constructor() {
        this.canvas = new jsnake.display('book');
    ...
    async _addimg(key, data) {
        ...
        for (let i = 0; i < data[key].length; i++) {
            ...
            let imge = await jsnake.image.scale(img, [1, 1]);
    ...
    async init() {
        ...
        this.font = new jsnake.font(this.screen, 'enchant', clamp(10, window.innerWidth / 153.6, 16));
        this.fonttext = new jsnake.font(this.screen, 'mc', 16)
        await this.loaddata();

        this.skills.forEach(async (dict) => {
            const img = await jsnake.image.imageload(dict.image)
            dict.image = img;
        })
